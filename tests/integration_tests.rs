...
mod tests {
    ...
    fn test_modem_state_initialization() {
        log_test_start("modem_state_initialization");
        ...
        info!("Modem state initialized - connection: {:?}, advertising: {:?}", 
              state.get_connection().is_some(), state.get_advertising_state());
        
        log_test_pass("modem_state_initialization");
    }

    #[test]
    fn test_uuid_base_registration() {
        log_test_start("uuid_base_registration");
        ...
        info!("Registered UUID base with handle: {}", handle);
        ...
        log_test_pass("uuid_base_registration");
    }

    #[test]
    fn test_max_uuid_bases() {
        log_test_start("max_uuid_bases");
        ...
        for i in 0..4 {
            ...
            info!("Registered UUID base {} with handle: {}", i, i);
        }
        
        info!("All 4 UUID base slots filled");
        ...
        match result {
            Err(StateError::UuidBasesExhausted) => {
                info!("Correctly rejected 5th UUID base registration");
            },
            other => panic!("Expected UuidBasesExhausted error, got: {:?}", other),
        }
        
        log_test_pass("max_uuid_bases");
    }

    #[test]
    fn test_service_registration() {
        log_test_start("service_registration");
        ...
        info!("UUID base registered with handle: {}", base_handle);
        ...
        assert!(result.is_ok());
        info!("Service added with handle: {}", service_handle);
        
        // Verify we can retrieve the service
        let retrieved = state.get_service(service_handle);
        assert!(retrieved.is_some());
        
        log_test_pass("service_registration");
    }

    #[test]
    fn test_command_error_conversions() {
        log_test_start("command_error_conversions");
        ...
        match cmd_error {
            CommandError::StateError(StateError::UuidBasesExhausted) => {
                info!("StateError::UuidBasesExhausted correctly converts to CommandError::StateError");
            },
            other => panic!("Expected StateError(UuidBasesExhausted), got: {:?}", other),
        }
        
        log_test_pass("command_error_conversions");
    }

    #[test]
    fn test_multiple_uuid_registrations() {
        log_test_start("multiple_uuid_registrations");
        ...
        assert_eq!(handle1, 0);
        info!("Registered UUID base 1 with handle: {}", handle1);
        
        // Register second UUID base  
        let uuid2 = [0x02; 16];
        let handle2 = state.register_uuid_base(uuid2).unwrap();
        assert_eq!(handle2, 1);
        info!("Registered UUID base 2 with handle: {}", handle2);
        ...
        info!("Both UUID bases can be retrieved correctly");
        
        log_test_pass("multiple_uuid_registrations");
    }

    #[test]
    fn test_state_edge_cases() {
        log_test_start("state_edge_cases");
        
        let mut state = ModemState::new();
        
        // Test retrieving non-existent UUID base
        assert!(state.get_uuid_base(99).is_none());
        info!("Non-existent UUID base correctly returns None");
        ...
        for (i, &pattern) in patterns.iter().enumerate() {
            ...
            info!("Pattern {} registered with handle: {}", i, handle);
        }
        
        log_test_pass("state_edge_cases");
    }
}