...
mod tests {
    ...
    fn test_bond_state_consistency() {
        embassy_futures::block_on(async {
            ...
            if bonded_device_count().await < MAX_BONDED_DEVICES {
                let add2_result = add_bonded_device(conn_handle2, peer_addr2, addr_type2).await;
                assert!(add2_result.is_ok());

                // Verify state consistency with both bonds
                assert_eq!(bonded_device_count().await, 2);
                assert!(is_device_bonded(conn_handle1).await);
                assert!(is_device_bonded(conn_handle2).await);

                // Remove first bond
                let remove_result = remove_bonded_device(conn_handle1).await;
                ...
                let update_result = set_system_attributes(conn_handle2, &updated_sys_attr).await;
                assert!(update_result.is_ok());

                // Verify state remains consistent after update
                let updated_attrs = get_system_attributes(conn_handle2).await;
    ...
    fn test_bonding_error_handling() {
        embassy_futures::block_on(async {
            ...
            let set_result = set_system_attributes(conn_handle, &sys_attr).await;
            assert!(set_result.is_ok());

            // Verify system attributes were stored
            let retrieved = get_system_attributes(conn_handle).await;
            assert!(retrieved.is_some());
            assert_eq!(retrieved.unwrap().len(), 3);

            // Test setting system attributes for non-bonded device
            let invalid_handle = 999;
            let invalid_set_result = set_system_attributes(invalid_handle, &sys_attr).await;
            ...
            let remove_result = remove_bonded_device(conn_handle).await;
            assert!(remove_result.is_ok());
            assert!(!is_device_bonded(conn_handle).await);

            // Test removing non-existent device
            let invalid_remove_result = remove_bonded_device(conn_handle).await;
            ...
            for i in 0..MAX_BONDED_DEVICES {
                ...
                let result = add_bonded_device(handle, addr, 0).await;
                assert!(result.is_ok());
                handles.push(handle);
            }

            // This should fail - table is full
            let overflow_result = add_bonded_device(999, [0xFF; 6], 0).await;
            assert!(matches!(overflow_result, Err(BondingError::BondingTableFull)));

            // Clean up
            for handle in handles {
                let _ = remove_bonded_device(handle).await;
            }
        });
    }
    ...
    fn test_address_type_handling() {
        embassy_futures::block_on(async {
            ...
            let add1_result = add_bonded_device(conn_handle1, peer_addr, 0).await;
            assert!(add1_result.is_ok());

            // Verify device info contains correct address type
            let device1 = get_bonded_device_info(conn_handle1).await;
            ...
            let add2_result = add_bonded_device(conn_handle2, peer_addr, 1).await;
            assert!(add2_result.is_ok());

            // Verify device info contains correct address type
            let device2 = get_bonded_device_info(conn_handle2).await;
    ...
    fn test_empty_system_attributes() {
        embassy_futures::block_on(async {
            ...
            // Add bonded device
            let add_result = add_bonded_device(conn_handle, peer_addr, 0).await;
            assert!(add_result.is_ok());

            // Set empty system attributes
            let empty_attrs: &[u8] = &[];
            let set_result = set_system_attributes(conn_handle, empty_attrs).await;
            assert!(set_result.is_ok());
            ...
            let update_result = set_system_attributes(conn_handle, &non_empty_attrs).await;
            assert!(update_result.is_ok());

            // Verify attributes were updated
            let updated = get_system_attributes(conn_handle).await;
            ...
            // Set back to empty
            let clear_result = set_system_attributes(conn_handle, &[]).await;
            assert!(clear_result.is_ok());

            // Verify cleared
            let cleared = get_system_attributes(conn_handle).await;
    ...
    fn test_operations_before_initialization() {
        embassy_futures::block_on(async {
            ...
            let set_result = set_system_attributes(conn_handle, &sys_attrs).await;
            assert!(set_result.is_ok());
    ...
    fn test_peer_address_integrity() {
        ...
        proptest!(|(
            ...
            for (i, addr) in addresses.iter().take(MAX_BONDED_DEVICES).enumerate() {
                ...
                // Add bonded device
                let add_result = embassy_futures::block_on(add_bonded_device(conn_handle, *addr, addr_type));
                prop_assert!(add_result.is_ok());
                test_handles.push(conn_handle);

                // Verify device info integrity
                let device_info = embassy_futures::block_on(get_bonded_device_info(conn_handle));
                ...
                prop_assert!(embassy_futures::block_on(is_device_bonded(conn_handle)));
            }
    ...
    fn test_connection_handle_edge_cases() {
        ...
        proptest!(|(
            ...
            prop_assert!(!embassy_futures::block_on(is_device_bonded(non_existent_handle)));
            prop_assert!(embassy_futures::block_on(get_bonded_device_info(non_existent_handle)).is_none());

            let invalid_attr_result = embassy_futures::block_on(set_system_attributes(non_existent_handle, &[0x99]));
