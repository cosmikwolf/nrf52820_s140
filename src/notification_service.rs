...
pub async fn send_notification(
    ...
    loop {
        ...
        debug!("Received notification response for different request ID: {}", response.response_id);
    }
}
...
pub async fn send_indication(
    ...
    loop {
        ...
        debug!("Received indication response for different request ID: {}", response.response_id);
    }
}
...
async fn process_notification_request(request: &NotificationRequest) -> Result<(), NotificationError> {
    debug!("Processing {} for conn {} char {}", 
           if request.is_indication { "indication" } else { "notification" },
           request.conn_handle, request.char_handle);
    
    // Check if connection exists
    let connection_exists = crate::connection_manager::with_connection_manager(|mgr| {
        mgr.get_connection(request.conn_handle).is_some()
    });
    
    if !connection_exists {
        warn!("Attempted to send notification to unknown connection {}", request.conn_handle);
    ...
    warn!("Notification sending not yet implemented - would send {} bytes to conn {} char {}",
          request.data.len(), request.conn_handle, request.char_handle);
