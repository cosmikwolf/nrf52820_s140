...
use crate::ble::dynamic::{DynamicGattEvent, DynamicGattServer};
...
impl Server {
    ...
    async fn handle_gatt_event(&self, event: DynamicGattEvent) {
        match event {
            DynamicGattEvent::CharacteristicWrite {
                conn_handle,
                char_handle,
                data_len,
            } => {
                ...
                info!(
                    "Characteristic write: conn={}, char={}, len={}",
                    conn_handle, char_handle, data_len
                );
            }
            DynamicGattEvent::CccdWrite {
                conn_handle,
                char_handle,
                notifications,
                indications,
                ...
                let cccd_event = events::create_cccd_write_event(conn_handle, char_handle, notifications, indications);
