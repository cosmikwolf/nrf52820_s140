...
impl BondingStorage {
        ...
        match self.bonded_devices.get_mut(&conn_handle) {
            Some(device) => {
                ...
                if device.sys_attr_data.extend_from_slice(sys_attr_data).is_err() {
                    error!("BONDING: System attributes data too large for connection {}", conn_handle);
                    return Err(BondingError::InvalidData);
                }
                debug!("BONDING: Updated system attributes for connection {} ({} bytes)", 
                       conn_handle, sys_attr_data.len());
                Ok(())
            },
            None => {
                warn!("BONDING: Attempted to set system attributes for unknown device {}", conn_handle);
    ...
    fn get_system_attributes(&self, conn_handle: u16) -> Option<&[u8]> {
        self.bonded_devices.get(&conn_handle)
...
pub fn add_bonded_device(conn_handle: u16, peer_addr: [u8; 6], addr_type: u8) -> Result<(), BondingError> {
    cortex_m::interrupt::free(|_cs| unsafe {
        BONDING_STORAGE.as_mut().unwrap().add_bonded_device(conn_handle, peer_addr, addr_type)
    })
}
...
pub fn set_system_attributes(conn_handle: u16, sys_attr_data: &[u8]) -> Result<(), BondingError> {
    cortex_m::interrupt::free(|_cs| unsafe {
        BONDING_STORAGE.as_mut().unwrap().set_system_attributes(conn_handle, sys_attr_data)
    })
}
...
pub fn get_system_attributes(conn_handle: u16) -> Option<heapless::Vec<u8, MAX_SYS_ATTR_SIZE>> {
    cortex_m::interrupt::free(|_cs| unsafe {
        BONDING_STORAGE.as_ref().unwrap().get_system_attributes(conn_handle)
            .map(|data| {
...
pub fn remove_bonded_device(conn_handle: u16) -> Result<(), BondingError> {
    cortex_m::interrupt::free(|_cs| unsafe {
        BONDING_STORAGE.as_mut().unwrap().remove_bonded_device(conn_handle)
    })
}
...
pub fn is_device_bonded(conn_handle: u16) -> bool {
    cortex_m::interrupt::free(|_cs| unsafe {
        BONDING_STORAGE.as_ref().unwrap().bonded_devices.contains_key(&conn_handle)
    })
}