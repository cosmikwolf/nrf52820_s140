...
impl DynamicGattServer {
    ...
    async fn handle_write_event(
        ...
        if let Some(char_info) = with_registry(|registry| {
            registry.find_characteristic_by_cccd_handle(handle)
                .map(|c| *c)
        }) {
        ...
        if let Some(char_info) = with_registry(|registry| {
            registry.find_characteristic_by_value_handle(handle)
                .map(|c| *c)
        }) {
            debug!("Characteristic write on handle {}: {} bytes", handle, data.len());
            
            // Forward write event to host
            let write_event = events::create_gatts_write_event(
                conn_handle,
                handle,
                data,
            );
...
impl gatt_server::Server for DynamicGattServer {
    ...
    fn on_write(
        ...
        // Check if this is a characteristic value write
        if let Some(_char_info) = with_registry(|registry| {
            registry.find_characteristic_by_value_handle(handle)
                .map(|c| *c)
        }) {
            debug!("Characteristic write on handle {}: {} bytes", handle, data.len());
        ...
        debug!("Write to unknown handle {} (op: {:?}, offset: {}, len: {})", 
               handle, op, offset, data.len());
        None
    }
}
...
pub mod utils {
    ...
    pub fn add_characteristic_to_builder(
        ...
        debug!("Added characteristic - value_handle: {}, cccd_handle: {}", 
    ...
    pub fn send_indication(
        ...
        handle: u16,
        data: &[u8],
    ) -> Result<(), gatt_server::IndicateValueError> {
